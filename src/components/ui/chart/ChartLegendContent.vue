...
const payload = computed(() =>
  Object.entries(config.value).map(([key, value]) => {
    return {
      ...
      itemConfig: config.value[key] || value,
    }
  })
)
...
<template>
  <div v-if="containerSelector" :class="cn('flex items-center justify-center gap-4', verticalAlign === 'top' ? 'pb-3' : 'pt-3', props.class)">
    <div v-for="{ key, itemConfig } in payload" :key="key" :class="cn('flex items-center gap-1.5 [&>svg]:h-3 [&>svg]:w-3 [&>svg]:text-muted-foreground')">
